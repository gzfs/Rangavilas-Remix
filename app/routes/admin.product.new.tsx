...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (formData.uploadedImages) {
    formData.uploadedImages.forEach(async (uploadedImage: string) => {
      await tursoDB
        .insertInto("Image")
        .values({
          id: uuid(),
    ...
    if (formData.productTags) {
      formData.productTags.forEach(async (productTag: Keyword) => {
        await tursoDB
          .insertInto("KeywordProduct")
          .values({
            id: uuid(),
            keyword_id: productTag.id,
            product_id: productID,
          })
          .execute();
      });
...
export default function AddProduct() {
  const { availableCategories, availableTags } =
  ...
  return (
    <section className="max-w-[1000px] mx-auto py-10 px-10">
      <p className="pb-8 font-Outfit text-5xl font-bold text-[#333333]">
        Images
      </p>
      <div className="grid grid-cols-3 gap-4">
        <ImageUpload setImageImgurUrl={setImageUrlOne} />
        <ImageUpload setImageImgurUrl={setImageUrlTwo} />
        <ImageUpload setImageImgurUrl={setImageUrlThree} />
      </div>
      <Form
        method="POST"
        onSubmit={(eV) => {
