...
import { type OutputElementType, mergeUrls } from "~/utils/helper.server";
...
export async function loader(loaderArgs: LoaderFunctionArgs) {
  ...
  const sliderImages = (
    ...
    .filter((sliderImage) => {
      return (
        sliderImage.category_id === null && sliderImage.product_id === null
      );
    })
    .map((sliderImage) => {
...
export default function Index() {
  ...
  return (
    <>
      <Slider sliderImages={loaderData.sliderImages} isRounded={false} />
      <section className="mx-auto max-w-[1000px] px-10">
        ...
        <Products
          ...
          hydrateProducts={loaderData.initalProducts as OutputElementType[]}
        />
      </section>
    </>
  );
